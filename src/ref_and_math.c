...
void copy_ch_to_ref(CHANNELSETTINGS *dest, const CHANNELSETTINGS *src, uint32 *tracebuffer) {
    ...
    memcpy(dest->tracebuffer, src->tracebuffer, SAMPLE_COUNT);  // Skopíruj dáta do tracebufferu
      ...
      
    {
      ...
      //------------------------------------------------------------------------ 
        if(number == 1)
        {
          ...
          if((result = f_read(&viewfp, (uint8 *)ref1_tracebuffer, SAMPLE_COUNT, 0)) == FR_OK)
          {
        ...
        {
          //Load the channel 1 sample data      
          if((result = f_read(&viewfp, (uint8 *)ref2_tracebuffer, SAMPLE_COUNT, 0)) == FR_OK)
          {
            //Load the channel 2 sample data
        ...
        {
          //Load the channel 1 sample data      
          if((result = f_read(&viewfp, (uint8 *)ref3_tracebuffer, SAMPLE_COUNT, 0)) == FR_OK)
          {
            //Load the channel 2 sample data
        ...
        {
          //Load the channel 1 sample data      
          if((result = f_read(&viewfp, (uint8 *)ref4_tracebuffer, SAMPLE_COUNT, 0)) == FR_OK)
          {
            //Load the channel 2 sample data
        ...
        {
          //Load the channel 1 sample data      
          if((result = f_read(&viewfp, (uint8 *)ref5_tracebuffer, SAMPLE_COUNT, 0)) == FR_OK)
          {
            //Load the channel 2 sample data
            //if((result = f_read(&viewfp, (uint8 *)channel2_ref1_tracebuffer, 3000, 0)) == FR_OK);   
          } 
        ...
        {
          //Load the channel 1 sample data      
          if((result = f_read(&viewfp, (uint8 *)ref6_tracebuffer, SAMPLE_COUNT, 0)) == FR_OK)
          {
            //Load the channel 2 sample data
            //if((result = f_read(&viewfp, (uint8 *)channel2_ref2_tracebuffer, 3000, 0)) == FR_OK);   
          } 
        ...
        {
          //Load the channel 1 sample data      
          if((result = f_read(&viewfp, (uint8 *)ref7_tracebuffer, SAMPLE_COUNT, 0)) == FR_OK)
          {
            //Load the channel 2 sample data
            //if((result = f_read(&viewfp, (uint8 *)channel2_ref3_tracebuffer, 3000, 0)) == FR_OK);   
          } 
        ...
        {
          //Load the channel 1 sample data      
          if((result = f_read(&viewfp, (uint8 *)ref8_tracebuffer, SAMPLE_COUNT, 0)) == FR_OK)
          {
            //Load the channel 2 sample data
            //if((result = f_read(&viewfp, (uint8 *)channel2_ref4_tracebuffer, 3000, 0)) == FR_OK);   
          } 
  ...
  if(result == FR_OK)
  {
    ...
    { 
        ...
        //if((result = f_write(&viewfp, &scopesettings.ch_ref1, sizeof(CHANNELSETTINGS), 0)) == FR_OK)
      {
        ...
        if((result = f_write(&viewfp, (uint8 *)ref1_tracebuffer, SAMPLE_COUNT, 0)) == FR_OK)
        {
    ...
    { 
      ...
      if((result = f_write(&viewfp, viewfilesetupdata, sizeof(viewfilesetupdata), 0)) == FR_OK)
      {
        //Write the trace data to the file
        //Save the channel 1 raw sample data
        if((result = f_write(&viewfp, (uint8 *)ref2_tracebuffer, SAMPLE_COUNT, 0)) == FR_OK)
        {
          //Save the channel 2 raw sample data
    ...
    { 
      ...
      if((result = f_write(&viewfp, viewfilesetupdata, sizeof(viewfilesetupdata), 0)) == FR_OK)
      {
        //Write the trace data to the file
        //Save the channel 1 raw sample data
        if((result = f_write(&viewfp, (uint8 *)ref3_tracebuffer, SAMPLE_COUNT, 0)) == FR_OK)
        {
          //Save the channel 2 raw sample data
    ...
    { 
      ...
      if((result = f_write(&viewfp, viewfilesetupdata, sizeof(viewfilesetupdata), 0)) == FR_OK)
      {
        //Write the trace data to the file
        //Save the channel 1 raw sample data
        if((result = f_write(&viewfp, (uint8 *)ref4_tracebuffer, SAMPLE_COUNT, 0)) == FR_OK)
        {
          //Save the channel 2 raw sample data
    ...
    { 
      ...
      if((result = f_write(&viewfp, viewfilesetupdata, sizeof(viewfilesetupdata), 0)) == FR_OK)
      {
        //Write the trace data to the file
        //Save the channel 1 raw sample data
        if((result = f_write(&viewfp, (uint8 *)ref5_tracebuffer, SAMPLE_COUNT, 0)) == FR_OK)
        {
          //Save the channel 2 raw sample data
          //result = f_write(&viewfp, (uint8 *)channel2_ref4_tracebuffer, 3000, 0);
        }
    ...
    { 
      ...
      if((result = f_write(&viewfp, viewfilesetupdata, sizeof(viewfilesetupdata), 0)) == FR_OK)
      {
        //Write the trace data to the file
        //Save the channel 1 raw sample data
        if((result = f_write(&viewfp, (uint8 *)ref6_tracebuffer, SAMPLE_COUNT, 0)) == FR_OK)
        {
          //Save the channel 2 raw sample data
          //result = f_write(&viewfp, (uint8 *)channel2_ref4_tracebuffer, 3000, 0);
        }
    ...
    { 
      ...
      if((result = f_write(&viewfp, viewfilesetupdata, sizeof(viewfilesetupdata), 0)) == FR_OK)
      {
        //Write the trace data to the file
        //Save the channel 1 raw sample data
        if((result = f_write(&viewfp, (uint8 *)ref7_tracebuffer, SAMPLE_COUNT, 0)) == FR_OK)
        {
          //Save the channel 2 raw sample data
          //result = f_write(&viewfp, (uint8 *)channel2_ref4_tracebuffer, 3000, 0);
        }
    ...
    { 
      ...
      if((result = f_write(&viewfp, viewfilesetupdata, sizeof(viewfilesetupdata), 0)) == FR_OK)
      {
        //Write the trace data to the file
        //Save the channel 1 raw sample data
        if((result = f_write(&viewfp, (uint8 *)ref8_tracebuffer, SAMPLE_COUNT, 0)) == FR_OK)
        {
          //Save the channel 2 raw sample data
          //result = f_write(&viewfp, (uint8 *)channel2_ref4_tracebuffer, 3000, 0);
        }
