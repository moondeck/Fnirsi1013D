...
//CCU registers (verify on your board) 
#define CCU_BASE              0x01C20000UL
#define CCU_BUS_CLK_GATING0   REG32(CCU_BASE + 0x60)
#define CCU_BUS_SOFT_RST0     REG32(CCU_BASE + 0x2D0)

// WDT registers (verify address on your board) 
#define WDT_BASE              0x01C20CA0UL
#define WDT_CTRL              REG32(WDT_BASE + 0x00)
#define WDT_CFG               REG32(WDT_BASE + 0x04)
#define WDT_MODE              REG32(WDT_BASE + 0x08)

//bits we expect (bit 12 per Allwinner family - verify) 
#define WDT_BUS_BIT           (1 << 12)
...
void dump_regs(void)
{
    ...
    a = CCU_BUS_CLK_GATING0;
    b = CCU_BUS_SOFT_RST0;
    c = WDT_CTRL;
    d = WDT_MODE;
    // Replace these with your logging / uart print function 
               usb_send_uint("CLK_GATING0 ", a);
               usb_send_uint("SOFT_RST0 ", b);
               usb_send_uint("WDT_CTRL ", c);
               usb_send_uint("WDT_MODE ", d);


            usb_CDC_in_ep_callback();
    // uart_printf("CLK_GATING0=0x%08x\n", a);
    // uart_printf("SOFT_RST0 =0x%08x\n", b);
    // uart_printf("WDT_CTRL  =0x%08x\n", c);
    // uart_printf("WDT_MODE  =0x%08x\n", d);
    (void)a;(void)b;(void)c;(void)d;
}

int try_enable_wdt_clock(void)
{
    // Try set-gating + set deassert reset (some platforms require set, others clear) 
    uint32 before, after;

    before = CCU_BUS_CLK_GATING0;
    CCU_BUS_CLK_GATING0 = before | WDT_BUS_BIT;
    delay(1000);
    after = CCU_BUS_CLK_GATING0;
    if ((after & WDT_BUS_BIT) == 0) return 0;
*/
//********************

#define WDT_BASE        0x01C20C90
#define WDT_CTRL        (*(volatile uint32 *)(WDT_BASE + 0x00))
#define WDT_CFG         (*(volatile uint32 *)(WDT_BASE + 0x04))
#define WDT_MODE        (*(volatile uint32 *)(WDT_BASE + 0x08))

#define CCM_BASE        0x01C20000
#define BUS_CLK_GATING0 (*(volatile uint32 *)(CCM_BASE + 0x60))
#define BUS_SOFT_RST0   (*(volatile uint32 *)(CCM_BASE + 0x2C))

void wdt_enable_clock(void)
{
    // BIT 31 = Watchdog clock gate
...
/*
printf("CLK_GATING0 = %08X\n", BUS_CLK_GATING0);
printf("SOFT_RST0   = %08X\n", BUS_SOFT_RST0);
printf("WDT_MODE    = %08X\n", WDT_MODE);
printf("WDT_CTRL    = %08X\n", WDT_CTRL);
*/
void system_reset(void)
{
    ...
    // Set shortest timeout (default is 0 -> ~0.5s)
    WDT_CFG = 0x00000000;

    // Enable WDT + enable reset output
    WDT_MODE = (1 << 0) | (1 << 1);
    
    usb_send_uint("WDT_MODE =  ", WDT_MODE); usb_CDC_in_ep_callback();

    // Start the watchdog
    WDT_CTRL = 0x01;
    
    usb_send_uint("WDT_CTRL =  ", WDT_CTRL); usb_CDC_in_ep_callback();

    while (1) {}  // do ~0.5 s príde HARD RESET
}
...
void scope_trigger_settings(int mode)
{
  ...
  display_set_fg_color(BLACK_COLOR); //LIGHTGREY_COLOR       
  ...
  display_fill_rounded_rect(TRIGGER_BUTTON_XPOS+TRIGGER_BUTTON_WIDTH, TRIGGER_BUTTON_YPOS, 48, 35, 2);//31+48 79
...
void scope_generator_settings(int mode)
{
  ...
  //buffer for text frequency
  char buffer[16];

  //Clear the area first light gray
  display_set_fg_color(BLACK_COLOR); //LIGHTGREY_COLOR  
 //display_set_fg_color(LIGHTGREY_COLOR); //LIGHTGREY_COLOR    
  display_fill_rounded_rect(GEN_BUTTON_XPOS, GEN_BUTTON_YPOS, GEN_BUTTON_BG_WIDTH, GEN_BUTTON_BG_HEIGHT, 2); //BGW 62
  ...
  if(mode == 0)
  {
    //Inactive, green menu button
    display_set_fg_color(GREEN_COLOR);
    display_set_bg_color(BLACK_COLOR); //LIGHTGREY_COLOR 
  }
  else
  {
    //Active, magenta menu button
  ...
  //Fill the button (with wave)
  //display_draw_rounded_rect(GEN_BUTTON_XPOS, GEN_BUTTON_YPOS, GEN_BUTTON_WIDTH, GEN_BUTTON_HEIGHT, 2);
  display_draw_rounded_rect(GEN_BUTTON_XPOS, GEN_BUTTON_YPOS, GEN_BUTTON_WIDTH-3, 16, 2);
  //display_draw_rounded_rect(GEN_BUTTON_XPOS, GEN_BUTTON_YPOS, GEN_BUTTON_BG_WIDTH, 10, 2);
  ...
  //Display the icon with the set colors
  //display_copy_icon_use_colors(Generator_menu_icon, GEN_BUTTON_XPOS+2, GEN_BUTTON_YPOS+6, 24, 24);//
  ...
  //Fill the settings background
  display_fill_rounded_rect(GEN_BUTTON_XPOS, GEN_BUTTON_YPOS, GEN_BUTTON_BG_WIDTH, 35, 2);//28+33=61/
  //display_fill_rounded_rect(GEN_BUTTON_XPOS+GEN_BUTTON_WIDTH, GEN_BUTTON_YPOS, 33, 35, 2);//28+33=61
  ...
  //Check on which trigger mode is set
  switch(fpgasettings.gen_wave_select)
  {
    case 0:
      modetext = "   Sine"; 
      break;
 ...
 // {
  ...
  //display_text(GEN_BUTTON_XPOS+31, GEN_BUTTON_YPOS+2, "100%");//+30 +1
  
//      extern uint32 a,b,c,d,e,f;
    
  //  fpgasettings.gen_duty = d;
  //  fpga_set_generator_duty(fpgasettings.gen_duty); 
    
//  fpgasettings.gen_freq = e;
//  fpga_set_generator_freq(fpgasettings.gen_freq);
  
  //convert duty number to text with %
  make_value_to_text(buffer, fpgasettings.gen_duty, '%');
  
  display_text(GEN_BUTTON_XPOS+25, GEN_BUTTON_YPOS+2, buffer);//+30 +1
  
  //display_text(GEN_BUTTON_XPOS+27, GEN_BUTTON_YPOS+2, "100%");//+30 +1
 //display_text(GEN_BUTTON_XPOS+27, GEN_BUTTON_YPOS+10, "100%");//+30 +1

  //Check which generator mode is selected
                               //SQUARE 
  if((fpgasettings.gen_wave_select==0)&&(!mode)) display_set_fg_color(ORANGE_COLOR);
                              //SINE
  else if((fpgasettings.gen_wave_select==1)&&(!mode)) display_set_fg_color(GREEN_COLOR);
                                //TRIANGLE
  else if((fpgasettings.gen_wave_select==2)&&(!mode)) display_set_fg_color(YELLOW_COLOR);
 
  //convert freq number to text with dot and Hz kHz MHz
  make_freq_to_text(buffer, fpgasettings.gen_freq);
  //display_text(GEN_MENU_XPOS + 68, GEN_MENU_YPOS + 132, buffer);
  display_text(GEN_BUTTON_XPOS+6, GEN_BUTTON_YPOS+19, buffer);//
  
  if((fpgasettings.gen_enable)&&(!mode))
  {
    display_set_fg_color(GREEN_COLOR);
    display_draw_rounded_rect(GEN_BUTTON_XPOS, GEN_BUTTON_YPOS, GEN_BUTTON_BG_WIDTH, GEN_BUTTON_BG_HEIGHT, 2);
  }
  else if((!fpgasettings.gen_enable)&&(!mode))
  {
    display_set_fg_color(RED_COLOR);
    display_draw_rounded_rect(GEN_BUTTON_XPOS, GEN_BUTTON_YPOS, GEN_BUTTON_BG_WIDTH, GEN_BUTTON_BG_HEIGHT, 2);
  }
  ...
  //display_text(GEN_BUTTON_XPOS+31, GEN_BUTTON_YPOS+22, "1kHz");//+30 +1
...
void scope_channel_probe_magnification_select(PCHANNELSETTINGS settings)
{
 ...
 //Check if magnification is 1x
  ...
  display_text(settings->menuxpos + 133, CH_MENU_YPOS + 290, "10");// +27 na 10x
...
//  uint32 xend;
  ...
  */
  ...
  scope_channel_CLEAR_REF_ALL_item(settings, 0);
  ...
  scope_channel_LOAD_REF_ALL_item(settings, 0);
  ...
  scope_channel_SAVE_REF_ALL_item(settings, 0);
...
void scope_channel_CLEAR_REF_ALL_item(PCHANNELSETTINGS settings, int mode)//set
{
  uint32 xstart;
  xstart = settings->menuxpos + CH_REF_MENU_XPOS + 74;
  ...
  display_text(xstart + 20, CH_REF_MENU_YPOS + 209, "ALL");//25
}
...
void scope_channel_LOAD_REF_ALL_item(PCHANNELSETTINGS settings, int mode)//set
{
  uint32 xstart;
  xstart = settings->menuxpos + CH_REF_MENU_XPOS + 142;
...
void scope_channel_SAVE_REF_ALL_item(PCHANNELSETTINGS settings, int mode)//set
{
  uint32 xstart;
  xstart = settings->menuxpos + CH_REF_MENU_XPOS + 210;
...
{
  //extern char numbuf[12];
  //menuxpos = menuxpos + KEY_MENU_XPOS;
  ...
  if(holdoffmenuopen)     display_text(KEY_MENU_XPOS + 20, KEY_MENU_YPOS + 7, "TRIGGER                   Set hold off time");//117
  else if(freqmenuopen)   display_text(KEY_MENU_XPOS + 20, KEY_MENU_YPOS + 7, "GENERATOR                 Set frequency");//117
  else if(dutymenuopen)   display_text(KEY_MENU_XPOS + 20, KEY_MENU_YPOS + 7, "GENERATOR                 Set duty");//117
  else if(amplmenuopen)   display_text(KEY_MENU_XPOS + 20, KEY_MENU_YPOS + 7, "GENERATOR                 Set amplitude");//117
  else if(offsetmenuopen) display_text(KEY_MENU_XPOS + 20, KEY_MENU_YPOS + 7, "GENERATOR                 Set offset");//117
  else if(phasemenuopen)  display_text(KEY_MENU_XPOS + 20, KEY_MENU_YPOS + 7, "GENERATOR                 Set phase");//117
  ...
  //Clear the boxes for the not selected items 
  display_fill_rect(KEY_MENU_XPOS +   5, KEY_MENU_YPOS + 284, 90, 50);  //MIN
  ...
  display_text(KEY_MENU_XPOS + 38,  KEY_MENU_YPOS +  310, "MIN"); //38-300
  //Set color for the text
  display_set_fg_color(DARKGREEN_COLOR);
  display_text(KEY_MENU_XPOS + 126, KEY_MENU_YPOS +  310, "Default"); //126
  //Set color for the text
  display_set_fg_color(RED_COLOR);
  display_text(KEY_MENU_XPOS + 233, KEY_MENU_YPOS +  310, "MAX"); //233
  ...
  if(freqmenuopen)
  {
    display_text(KEY_MENU_XPOS + 38,  KEY_MENU_YPOS +  293, "1 Hz"); //38-300
    display_text(KEY_MENU_XPOS + 135, KEY_MENU_YPOS +  293, "1 kHz"); //126
    display_text(KEY_MENU_XPOS + 230, KEY_MENU_YPOS +  293, "5 Mhz"); //233
  }
  
  else if(dutymenuopen)
  {
    display_text(KEY_MENU_XPOS + 40,  KEY_MENU_YPOS +  293, "0 %"); //38-300
    display_text(KEY_MENU_XPOS + 137, KEY_MENU_YPOS +  293, "50 %"); //135
    display_text(KEY_MENU_XPOS + 230, KEY_MENU_YPOS +  293, "100 %"); //230
  }
  ...
  uint8 number_len = 0;
  extern char numbuffer[NUMBUF_MAX];
  extern uint8 numbuffer_len;

  
  if (numbuffer_len>0) number_len = numbuffer_len - 1; else number_len = 0;              
  display_text(KEY_MENU_XPOS + 269-(number_len*12), KEY_MENU_YPOS + 34, numbuffer); //200
  //buffer_clear(); 
  ...
  display_slide_right_rect_onto_screen(KEY_MENU_XPOS, KEY_MENU_YPOS, KEY_MENU_WIDTH, KEY_MENU_HEIGHT, 61000);
}
...
void scope_keyboard_select(void)
{
  ...
  for(i=0;i<((sizeof(keyboard_texts) / sizeof(int8 *)));i++)
  {
    ...
    //Selected texts in white
    if(((i == 13)||(i == 14)||(i > 15))&&(genmenuopen==1)) display_set_fg_color(GREY_COLOR); else display_set_fg_color(WHITE_COLOR);
...
void scope_open_acquisition_menu(void)
{
  ...
  display_set_fg_color(BLACK_COLOR);
  display_draw_horz_line(ACQ_MENU_YPOS +  155, ACQ_MENU_XPOS + 8, ACQ_MENU_XPOS + ACQ_MENU_WIDTH - 84);
  
  //display_set_fg_color(BLACK_COLOR);
  display_draw_horz_line(ACQ_MENU_YPOS +  211, ACQ_MENU_XPOS + 8, ACQ_MENU_XPOS + ACQ_MENU_WIDTH - 8);//-84
  
  display_draw_vert_line(ACQ_MENU_XPOS +  304, ACQ_MENU_YPOS + 8, ACQ_MENU_YPOS + ACQ_MENU_HEIGHT - 8);
  ...
  display_text(ACQ_MENU_XPOS + 317, ACQ_MENU_YPOS +   8, "Memory"); //tab +6   
  //display_text(ACQ_MENU_XPOS + 324, ACQ_MENU_YPOS +   20, "deep"); //tab +6 

  display_text(ACQ_MENU_XPOS + 325, ACQ_MENU_YPOS +   216, "Trace"); //tab +6 //323   
  ...
  scope_acquisition_memory_deep_select();
  scope_acquisition_trace_select();
...
void scope_acquisition_memory_deep_select(void)
{                  
  //Select the font for the texts
  //display_set_font(&font_3);
  ...
  //Set middle grey color for the boxes behind the not selected texts & highlight the selected item
  display_set_fg_color(MIDLEGREY_COLOR);
      
  //Clear the boxes for the not selected items
  for(i=0;i<(sizeof(memory_deep_texts) / sizeof(int8 *));i++)
  { 
    //Get the position of the item
    x = 315 + ACQ_MENU_XPOS;
    y = (i * 36) + 33 + ACQ_MENU_YPOS;

    //Set yellow color for the box behind the selected text
    if(i == (scopesettings.samplecount/3000) - 1) display_set_fg_color(YELLOW_COLOR); else display_set_fg_color(MIDLEGREY_COLOR);
      
    display_fill_rounded_rect(x, y, 54, 25, 2);//53
  }
  ...
  for(i=0;i<(sizeof(memory_deep_texts) / sizeof(int8 *));i++)
  {
    ...
    //Check if the current text is the selected on
    if(c != ((scopesettings.samplecount/3000) - 1))
    {
      //Available but viable not selected texts in white
      display_set_fg_color(WHITE_COLOR);
    }
    ...
    //Calculate the position of this text
    x = 315 + memory_deep_texts_x_offsets[c];
    y = (c * 36) +  38;

    //Display the text from the table
    display_text(ACQ_MENU_XPOS + x, ACQ_MENU_YPOS + y, (int8 *)memory_deep_texts[c]);
  }
...
void scope_acquisition_trace_select(void)
{                  
  //Select the font for the texts
  //display_set_font(&font_3);
  ...
  //Clear the boxes for the not selected items
  for(i=0;i<(sizeof(trace_texts) / sizeof(int8 *));i++)
  {

    x = 315 + ACQ_MENU_XPOS;
    y = (i * 36) +  241 + ACQ_MENU_YPOS;//228

    //Set yellow color for the box behind the selected text
    if(i == (scopesettings.ACQ_trace)) display_set_fg_color(ORANGE_COLOR); else display_set_fg_color(MIDLEGREY_COLOR);
  ...
  for(i=0;i<(sizeof(trace_texts) / sizeof(int8 *));i++)
  {
    ...
    //Calculate the position of this text
    x = 315 + trace_texts_x_offsets[c];
    y = (c * 36) +  246; //+5

    //Display the text from the table
    display_text(ACQ_MENU_XPOS + x, ACQ_MENU_YPOS + y, (int8 *)trace_texts[c]);
  }
}
...
void scope_open_trigger_menu(void)
{
  ...
  display_draw_horz_line(326, xstart, xend);////270 716
  ...
  display_text(TRIGGER_MENU_XPOS+10, TRIGGER_MENU_YPOS+178, "Trigger");//224
  display_text(TRIGGER_MENU_XPOS+10, TRIGGER_MENU_YPOS+197, "position");//243 position 
  ...
  display_text(TRIGGER_MENU_XPOS+10, TRIGGER_MENU_YPOS+290, "Always");//224
  display_text(TRIGGER_MENU_XPOS+10, TRIGGER_MENU_YPOS+309, "50%");//243
  
  //Display the icon with the set colors
  display_copy_icon_use_colors(trigger_50_percent_icon, TRIGGER_MENU_XPOS+75, TRIGGER_MENU_YPOS+294, 24, 24);//296 //184

  //Display the actual settings
  scope_trigger_mode_select();
  scope_trigger_edge_select();
  scope_trigger_channel_select();
  scope_trigger_position_select();
...
void scope_trigger_edge_select(void)
{
  ...
  display_set_fg_color(DARKGREY_COLOR);
  scope_trigger_symbol(TRIGGER_MENU_XPOS+66, TRIGGER_MENU_YPOS+59);
}
...
void scope_trigger_position_select(void)
{
//Select the font for the texts
  display_set_font(&font_3);
  ...
  //Check if trigger point is 25 50 75%
  if(fpgasettings.settriggerpoint == scopesettings.samplecount / 4)//25%
  {
    //1 so dark grey box behind 50% text
    display_fill_rounded_rect(TRIGGER_MENU_XPOS+121, TRIGGER_MENU_YPOS+185, 33, 22, 2);//118
    //2 so dark grey box behind 75% text
    display_fill_rounded_rect(TRIGGER_MENU_XPOS+172, TRIGGER_MENU_YPOS+185, 33, 22, 2);//166
    display_set_fg_color(GREY_COLOR);//GREY_COLOR
    display_draw_rounded_rect(TRIGGER_MENU_XPOS+121, TRIGGER_MENU_YPOS+185, 33, 22, 2);//118
    display_draw_rounded_rect(TRIGGER_MENU_XPOS+172, TRIGGER_MENU_YPOS+185, 33, 22, 2);
  }
  else if(fpgasettings.settriggerpoint == scopesettings.samplecount / 2)//50%
  {
    //2 so dark grey box behind 25% text
    display_fill_rounded_rect(TRIGGER_MENU_XPOS+70, TRIGGER_MENU_YPOS+185, 33, 22, 2);
    //2 so dark grey box behind 75% text
    display_fill_rounded_rect(TRIGGER_MENU_XPOS+172, TRIGGER_MENU_YPOS+185, 33, 22, 2);
    display_set_fg_color(GREY_COLOR);//GREY_COLOR
    display_draw_rounded_rect(TRIGGER_MENU_XPOS+70, TRIGGER_MENU_YPOS+185, 33, 22, 2);
    display_draw_rounded_rect(TRIGGER_MENU_XPOS+172, TRIGGER_MENU_YPOS+185, 33, 22, 2);
  }
  else if(fpgasettings.settriggerpoint == scopesettings.samplecount - 1000)//75%
  {
    //2 so dark grey box behind 25% text
    display_fill_rounded_rect(TRIGGER_MENU_XPOS+70, TRIGGER_MENU_YPOS+185, 33, 22, 2);
    //1 so dark grey box behind 50% text
    display_fill_rounded_rect(TRIGGER_MENU_XPOS+121, TRIGGER_MENU_YPOS+185, 33, 22, 2);//188
    display_set_fg_color(GREY_COLOR);//GREY_COLOR
    display_draw_rounded_rect(TRIGGER_MENU_XPOS+70, TRIGGER_MENU_YPOS+185, 33, 22, 2);
    display_draw_rounded_rect(TRIGGER_MENU_XPOS+121, TRIGGER_MENU_YPOS+185, 33, 22, 2);
  }
  ...
  //Check if trigger point is 25 50 75%
  if(fpgasettings.settriggerpoint == scopesettings.samplecount / 4)//0-ch1
  {
    //Set channel color for the box behind the selected text
    display_set_fg_color(LDARKBLUE_COLOR);
    //1 so trigger color box behind CH1 text
  ...
  else if(fpgasettings.settriggerpoint == scopesettings.samplecount / 2)//1-ch2
  {
    display_set_fg_color(DARKGREEN_COLOR);
    //2 so trigger color box behind CH2 text
    display_fill_rounded_rect(TRIGGER_MENU_XPOS+121, TRIGGER_MENU_YPOS+185, 33, 22, 2);//188
  }
  else if(fpgasettings.settriggerpoint == scopesettings.samplecount - 1000)
  {
    display_set_fg_color(LIGHTRED_COLOR);
    //2 so trigger color box behind CH2 text
    display_fill_rounded_rect(TRIGGER_MENU_XPOS+172, TRIGGER_MENU_YPOS+185, 33, 22, 2);//188
  }

   //Check if trigger point is 25%
  if(fpgasettings.settriggerpoint == scopesettings.samplecount / 4)
  {
    //1 so black CH1 text
    display_set_fg_color(BLACK_COLOR);
  }
  ...
  //Display the 25% text
  display_text(TRIGGER_MENU_XPOS+75, TRIGGER_MENU_YPOS+188, "25%");//191
  //-----------------------------------------------------

  //Check if trigger point is 50%
  if(fpgasettings.settriggerpoint == scopesettings.samplecount / 2)
  {
    //2 so black CH2 text
    display_set_fg_color(BLACK_COLOR);
  }
  ...
  //Display the 50% text
  display_text(TRIGGER_MENU_XPOS+126, TRIGGER_MENU_YPOS+188, "50%");
  
    //-----------------------------------------------------

  //Check if trigger point is 75%
  if(fpgasettings.settriggerpoint == scopesettings.samplecount - 1000)
  {
    // so black EXT text
    display_set_fg_color(BLACK_COLOR);
  }
  ...
  //Display the 75% text
  display_text(TRIGGER_MENU_XPOS+177, TRIGGER_MENU_YPOS+188, "75%");
}
...
{
  //Select the font for the texts
  //display_set_font(&font_3); //treba? 21.11.2025

  //Set dark grey color for the box behind the not selected text
  //display_set_fg_color(DARKGREY_COLOR); //treba? 21.11.2025
  
  //Show the state (if always trige 50 on or off)
  scope_display_slide_button(TRIGGER_MENU_XPOS + 140, TRIGGER_MENU_YPOS + 296, scopesettings.alwaystrigger50, GREEN_COLOR);//140 186
}
...
void scope_trigger_symbol(int x, int y)
{
  ...
  display_fill_rect(x+66, y+19, 11, 14);
  
  //White text if in normal mode
  display_set_fg_color(WHITE_COLOR);
  
  //Draw the trigger edge symbol
  display_draw_vert_line(x+71, y+20, y+31);

  //Draw the arrow based on the selected edge
  if(scopesettings.triggeredge == 0)
  {
    //rising edge
    display_draw_horz_line(y+20, x+71, x+74);
    display_draw_horz_line(y+31, x+68, x+71);
    display_draw_horz_line(y+25, x+70, x+72);
    display_draw_horz_line(y+26, x+69, x+73);
  }
  else
  {
    //falling edge
    display_draw_horz_line(y+20, x+68, x+71);
    display_draw_horz_line(y+31, x+71, x+74);
    display_draw_horz_line(y+25, x+69, x+73);
    display_draw_horz_line(y+26, x+70, x+72);
  }
}
...
void scope_open_generator_menu(void)
{
  ...
  //Setup the menu in a separate buffer to be able to slide it onto the screen
  display_set_screen_buffer(displaybuffertmp);//1 //display_set_source_buffer(displaybuffer1);//1

  //Draw the background in dark grey
  display_set_fg_color(DARKGREY_COLOR);

  //Fill the background
  display_fill_rect(GEN_MENU_XPOS, GEN_MENU_YPOS, GEN_MENU_WIDTH, GEN_MENU_HEIGHT);

  //Draw the edge in a lighter grey
  display_set_fg_color(LIGHTGREY_COLOR);

  //Draw the edge
  display_draw_rect(GEN_MENU_XPOS, GEN_MENU_YPOS, GEN_MENU_WIDTH, GEN_MENU_HEIGHT);
  
    //Line start and end x positions
  xstart = GEN_MENU_XPOS + 10;
  xend   = GEN_MENU_XPOS + GEN_MENU_WIDTH - 10;

  //Two black lines between the settings step 56
  display_set_fg_color(BLACK_COLOR);
  display_draw_horz_line(102, xstart, xend);//
  display_draw_horz_line(158, xstart, xend);//
  display_draw_horz_line(214, xstart, xend);//
  display_draw_horz_line(270, xstart, xend);//
  display_draw_horz_line(326, xstart, xend);//
  display_draw_horz_line(382, xstart, xend);//
  ...
  //Display the texts
  display_text(GEN_MENU_XPOS+10, GEN_MENU_YPOS+20,  "Power");//
  display_text(GEN_MENU_XPOS+10, GEN_MENU_YPOS+76,  "Wave");// 
  display_text(GEN_MENU_XPOS+10, GEN_MENU_YPOS+132, "Freq");//
  display_text(GEN_MENU_XPOS+10, GEN_MENU_YPOS+188, "Duty");//
  display_text(GEN_MENU_XPOS+10, GEN_MENU_YPOS+244, "Ampl");// 
  display_text(GEN_MENU_XPOS+10, GEN_MENU_YPOS+300, "Offset");//
  display_text(GEN_MENU_XPOS+10, GEN_MENU_YPOS+356, "Phase");//
  
   //display_copy_icon_use_colors ramik olkolo ikony generatora
  //display_draw_rounded_rect(GEN_MENU_XPOS+77, GEN_MENU_YPOS+11, 30, 32, 2);
  
  //Background icon color dark grey
  display_set_bg_color(DARKGREY_COLOR);
  
  //Display the icon with the set colors
  //display_copy_icon_use_colors(Generator_menu_icon, GEN_MENU_XPOS+80, GEN_MENU_YPOS+15, 24, 24);//24*24176//+21 299
  
  //Display the icon with the set colors
  //display_copy_icon_use_colors(trigger_50_percent_icon, GEN_MENU_XPOS+75, GEN_MENU_YPOS+184, 24, 24);

  //Display the actual settings
  gen_enable_select(); //gen_source_select();
  gen_wave_select();
  gen_freq_select();
  gen_duty_select();
  gen_ampl_select();
  ...
  //Slide the image onto the actual screen. The speed factor makes it start fast and end slow, Smaller value makes it slower.
  //display_slide_top_rect_onto_screen(560, 46, 166, 246, 56415);//186
  display_slide_top_rect_onto_screen(GEN_MENU_XPOS, GEN_MENU_YPOS, GEN_MENU_WIDTH, GEN_MENU_HEIGHT, 56415);
}


//==============================================================================
void gen_enable_select(void) //genmenuopen
{
    //Select the font for the texts
  //display_set_font(&font_3);
  ...
  //display_set_fg_color(DARKGREY_COLOR); //treba? 21.11.2025
     ...
     //extern uint32 a,b,c,d,e,f,g,h,i,j,k;
  
  // add source  interny - externy , ch1 and ch2
  //display_text(GEN_MENU_XPOS+10, GEN_MENU_YPOS+356, "Source");//
     
  //Show the state (generator on or off)
  scope_display_slide_button(TRIGGER_MENU_XPOS + 162, TRIGGER_MENU_YPOS + 17, fpgasettings.gen_enable, GREEN_COLOR);//135 upravitt na 140 aj 50% triger a button
  
}
//==============================================================================
void gen_wave_select(void) //genmenuopen
{
  //Select the font for the texts
  display_set_font(&font_3);
  ...
  {
    case 0:                                  //SINE
      //dark grey SINE and TRIANGLE
      display_fill_rounded_rect(GEN_MENU_XPOS+117, GEN_MENU_YPOS+73, 38, 22, 2);//36 57 20 38
      display_fill_rounded_rect(GEN_MENU_XPOS+165, GEN_MENU_YPOS+73, 42, 22, 2);
      display_set_fg_color(GREY_COLOR);//GREY_COLOR
      display_draw_rounded_rect(GEN_MENU_XPOS+117, GEN_MENU_YPOS+73, 38, 22, 2);//628 57 20 38
      display_draw_rounded_rect(GEN_MENU_XPOS+165, GEN_MENU_YPOS+73, 42, 22, 2);//692 56
      break;//SINE


    case 1:                                 //SQUARE
      //dark grey SQUARE and TRIANGLE
      display_fill_rounded_rect(GEN_MENU_XPOS+65,  GEN_MENU_YPOS+73, 42, 22, 2);//65 17na74 47 22
      display_fill_rounded_rect(GEN_MENU_XPOS+165, GEN_MENU_YPOS+73, 42, 22, 2);//54 56
      display_set_fg_color(GREY_COLOR);//GREY_COLOR
      display_draw_rounded_rect(GEN_MENU_XPOS+65,  GEN_MENU_YPOS+73, 42, 22, 2);//47 56
      display_draw_rounded_rect(GEN_MENU_XPOS+165, GEN_MENU_YPOS+73, 42, 22, 2);//54 56
      break;

    default:                                //TRIANGLE
      //dark grey SINE and SQUARE
      display_fill_rounded_rect(GEN_MENU_XPOS+65, GEN_MENU_YPOS+73, 42, 22, 2);//629 57 20 38
      display_fill_rounded_rect(GEN_MENU_XPOS+117, GEN_MENU_YPOS+73, 38, 22, 2);//661
      display_set_fg_color(GREY_COLOR);//GREY_COLOR
      display_draw_rounded_rect(GEN_MENU_XPOS+65, GEN_MENU_YPOS+73, 42, 22, 2);//661 56
      display_draw_rounded_rect(GEN_MENU_XPOS+117, GEN_MENU_YPOS+73, 38, 22, 2);//661
      break;
  }
  ...
  {
    case 0:
      //Highlight SQUARE mode
      display_set_fg_color(ORANGE_COLOR); 
      display_fill_rounded_rect(GEN_MENU_XPOS+65, GEN_MENU_YPOS+73, 42, 22, 2);//661 68 101 132
      break;
      
    case 1:
      //Highlight SINE mode
      display_set_fg_color(GREEN_COLOR);  
      display_fill_rounded_rect(GEN_MENU_XPOS+117, GEN_MENU_YPOS+73, 38, 22, 2);//629 57 20 38
      break;

    default:
      //Highlight TRIANGLE mode
      display_set_fg_color(YELLOW_COLOR);//DARKGREEN_COLOR
      display_fill_rounded_rect(GEN_MENU_XPOS+165, GEN_MENU_YPOS+73, 42, 22, 2);//692
      break;
  }

  //Check if generator mode is square
  if(fpgasettings.gen_wave_select == 0)
  {
    //Yes so black square text
    display_set_fg_color(BLACK_COLOR);
  }
  else
  {
    //White square text if in normal mode
    display_set_fg_color(WHITE_COLOR);
  }
  
  //Display the single text
  display_text(GEN_MENU_XPOS+67, GEN_MENU_YPOS+76,  "SQUR");//71

  //Check if generator mode is sine
  if(fpgasettings.gen_wave_select == 1)
  {
    //Yes so black sine text
    display_set_fg_color(BLACK_COLOR);
  }
  else
  {
    //White sine text if in generator mode
    display_set_fg_color(WHITE_COLOR);
  }
  
  //Display the sine text
  display_text(GEN_MENU_XPOS+121, GEN_MENU_YPOS+76, "SINE");//123


  //Check if generator mode is triangle
  if(fpgasettings.gen_wave_select == 2)
  {
    //Yes so black normal text
    display_set_fg_color(BLACK_COLOR);
  }
  ...
  {
    //White normal text if in triangle mode
    display_set_fg_color(WHITE_COLOR); 
  }

  //Display the triangle text
  display_text(GEN_MENU_XPOS+169, GEN_MENU_YPOS+76,  "TRGL");//695 155 33
}
//==============================================================================
void gen_freq_select(void)
{  
  //buffer for text frequency
  char buffer[16];
  ...
  //dark grey 
  display_fill_rounded_rect(GEN_MENU_XPOS+65,  GEN_MENU_YPOS+129, 92, 22, 2);//69-129-88-22-2//65 17na74 47 22      
  display_fill_rounded_rect(GEN_MENU_XPOS+165, GEN_MENU_YPOS+129, 42, 22, 2);//692 56
      
  display_set_fg_color(GREEN_COLOR);
  display_draw_rounded_rect(GEN_MENU_XPOS+65,  GEN_MENU_YPOS+129, 92, 22, 2);
  display_set_fg_color(GREY_COLOR);
  display_draw_rounded_rect(GEN_MENU_XPOS+165, GEN_MENU_YPOS+129, 42, 22, 2);//692 56

  //White normal text if in normal mode
  display_set_fg_color(WHITE_COLOR);

  make_freq_to_long_text(buffer, fpgasettings.gen_freq);
  display_text(GEN_MENU_XPOS + 68, GEN_MENU_YPOS + 132, buffer);

  //Display the normal text
  display_text(GEN_MENU_XPOS+169, GEN_MENU_YPOS+132, "1 kHz");//695 155 33
}
//==============================================================================
void gen_duty_select(void)
{  
  //buffer for text frequency
  char buffer[16];
  ...
  display_fill_rounded_rect(GEN_MENU_XPOS+90,  GEN_MENU_YPOS+185, 42, 22, 2);//69-129-88-22-2//65 17na74 47 22      
  display_fill_rounded_rect(GEN_MENU_XPOS+165, GEN_MENU_YPOS+185, 42, 22, 2);//692 56
      
  display_set_fg_color(YELLOW_COLOR);
  //display_draw_rounded_rect(GEN_MENU_XPOS+65,  GEN_MENU_YPOS+185, 92, 22, 2);
  display_draw_rounded_rect(GEN_MENU_XPOS+90,  GEN_MENU_YPOS+185, 42, 22, 2);//65//90
  display_set_fg_color(GREY_COLOR);
  display_draw_rounded_rect(GEN_MENU_XPOS+165, GEN_MENU_YPOS+185, 42, 22, 2);//692 56

  //White normal text if in normal mode
  display_set_fg_color(WHITE_COLOR);
    
    //fpgasettings.gen_duty = d;
    //fpga_set_generator_duty(fpgasettings.gen_duty);  //in Hz

  make_value_to_text(buffer, fpgasettings.gen_duty, '%');
  display_text(GEN_MENU_XPOS + 95, GEN_MENU_YPOS + 188, buffer);//70

  //Display the normal text
  display_text(GEN_MENU_XPOS+175, GEN_MENU_YPOS+188, "50%");//175
//display_text(GEN_MENU_XPOS+169, GEN_MENU_YPOS+132,  "1 kHz");//695 155 33
  
}
//==============================================================================
void gen_ampl_select(void)
{  
  //buffer for text frequency
  char buffer[16];
  ...
  display_set_fg_color(DARKGREY_COLOR);
                                //SINE
  //dark grey SQUARE and TRIANGLE
  display_fill_rounded_rect(GEN_MENU_XPOS+65,  GEN_MENU_YPOS+241, 92, 22, 2);//69-129-88-22-2//65 17na74 47 22      
  display_fill_rounded_rect(GEN_MENU_XPOS+165, GEN_MENU_YPOS+241, 42, 22, 2);//692 56
      
  display_set_fg_color(RED_COLOR);
  display_draw_rounded_rect(GEN_MENU_XPOS+65,  GEN_MENU_YPOS+241, 92, 22, 2);
  display_set_fg_color(GREY_COLOR);
  display_draw_rounded_rect(GEN_MENU_XPOS+165, GEN_MENU_YPOS+241, 42, 22, 2);//692 56
  ...
  //Display the normal text
  display_text(GEN_MENU_XPOS+169, GEN_MENU_YPOS+244, "5.00V");//695 155 33
}
//==============================================================================
void gen_offset_select(void)
{  
  //buffer for text frequency
  char buffer[16];
  ...
  display_set_fg_color(DARKGREY_COLOR);
                                //SINE
  //dark grey SQUARE and TRIANGLE
  display_fill_rounded_rect(GEN_MENU_XPOS+65,  GEN_MENU_YPOS+297, 92, 22, 2);//69-129-88-22-2//65 17na74 47 22      
  display_fill_rounded_rect(GEN_MENU_XPOS+165, GEN_MENU_YPOS+297, 42, 22, 2);//692 56
      
  display_set_fg_color(BLUE_COLOR);
  display_draw_rounded_rect(GEN_MENU_XPOS+65,  GEN_MENU_YPOS+297, 92, 22, 2);
  display_set_fg_color(GREY_COLOR);
  display_draw_rounded_rect(GEN_MENU_XPOS+165, GEN_MENU_YPOS+297, 42, 22, 2);//692 56
  ...
  //Display the normal text
  display_text(GEN_MENU_XPOS+169, GEN_MENU_YPOS+300,  "2.50V");//695 155 33
}
//==============================================================================
void gen_phase_select(void)
{  
  //buffer for text frequency
  char buffer[16];
  ...
  display_set_fg_color(DARKGREY_COLOR);
                                //SINE
  //dark grey SQUARE and TRIANGLE
  display_fill_rounded_rect(GEN_MENU_XPOS+65,  GEN_MENU_YPOS+353, 92, 22, 2);//69-129-88-22-2//65 17na74 47 22      
  display_fill_rounded_rect(GEN_MENU_XPOS+165, GEN_MENU_YPOS+353, 42, 22, 2);//692 56
      
  display_set_fg_color(ORANGE_COLOR);
  display_draw_rounded_rect(GEN_MENU_XPOS+65,  GEN_MENU_YPOS+353, 92, 22, 2);
  display_set_fg_color(GREY_COLOR);
  display_draw_rounded_rect(GEN_MENU_XPOS+165, GEN_MENU_YPOS+353, 42, 22, 2);//692 56
  ...
  //Display the normal text
  display_text(GEN_MENU_XPOS+182, GEN_MENU_YPOS+356,  "0");//169//172
  display_character(GEN_MENU_XPOS+182+8, GEN_MENU_YPOS+356, 176);//-1 ma byt //198
...
void scope_system_settings_screen_brightness_value(void)
{
  ...
  display_set_fg_color(YELLOW_COLOR);
  display_fill_rounded_rect(328, 67, 36, 16, 2);   //font 2
  //display_fill_rounded_rect(325, 66, 54, 20, 2);    // font 5
  
  //convert number to text with %
  make_value_to_text(buffer, scopesettings.screenbrightness, '%');

  //Display the number with  black color
  display_set_font(&font_2);//5
  display_set_fg_color(BLACK_COLOR);
  display_text(330, 68, buffer); 
  //display_text(325, 64, buffer); //font 5
}
...
void scope_system_settings_grid_brightness_value(void)
{
  ...
  //Draw the yellow background
  display_set_fg_color(YELLOW_COLOR);
  display_fill_rounded_rect(328, 124, 36, 16, 2);   //font 2
  //display_fill_rounded_rect(325, 123, 54, 20, 2);    // font 5
  
  //convert number to text with %
  make_value_to_text(buffer, scopesettings.gridbrightness, '%');

  //Display the number with  black color
  display_set_font(&font_2);//5
  display_set_fg_color(BLACK_COLOR);
  display_text(330, 125, buffer); 
  //display_text(325, 121, buffer); //font 5
}
 ...
 */
  ...
  */
  display_set_fg_color(DARKGREEN_COLOR);
  display_text(330, 20, "Total samples:");//j+135
  display_set_fg_color(WHITE_COLOR);
  display_decimal(430, 20, scopesettings.samplecount);
  
  display_set_fg_color(YELLOW_COLOR);
  display_text(330, 40, "FPGA version:");//j+135
  
  display_set_fg_color(WHITE_COLOR);
  display_hex(430, 40, 4, fpga_get_version());
  
  if(fpgasettings.fw_FPGA == 1) 
  { 
    display_text(430, 60, "fw: Fnirsi");
    fpgasettings.totalsamples = 3000; 
    ...
    display_set_fg_color(ORANGE_COLOR);
    display_text(330, 80,  "Total samples set to 3000");//j+135
    display_text(330, 100, "Trigger point set to   50%");//j+135
  }
  
  if(fpgasettings.fw_FPGA == 2) 
  { 
    display_text(430, 60, "fw: PECO");
    display_set_fg_color(BLUE_COLOR);
    display_text(430, 80, "AL3-A10");
  }
  
  if(fpgasettings.fw_FPGA == 3) 
  { 
    display_text(430, 60, "fw: PECO");
    display_set_fg_color(BLUE_COLOR);
    display_text(430, 80, "EF2-L45");
    //fpgasettings.totalsamples = 9000; 
    //scopesettings.samplecount = fpgasettings.totalsamples;
    //scopesettings.nofsamples = fpgasettings.totalsamples/2;

    display_set_fg_color(ORANGE_COLOR);
    display_text(330, 110,  "Total samples MAX 12000");//j+135
  }
    
  //display_decimal(430, 40, scopesettings.samplecount);
...
{
  //Draw the background in dark gray
  display_set_fg_color(DARKGREY_COLOR);
...
switch (scopesettings.source1_measures)
{
  ...
  display_text(375, 273, "Clear");  
  
  if(scopesettings.source1_measures == 0) display_text(253, 273, "CH1");
  if(scopesettings.source1_measures == 1) display_text(250, 242, "Ref 1");
  if(scopesettings.source1_measures == 2) display_text(312, 242, "Ref 2"); 
  if(scopesettings.source1_measures == 3) display_text(374, 242, "Ref 3");
  if(scopesettings.source1_measures == 4) display_text(436, 242, "Ref 4");
...
switch (scopesettings.source2_measures)
{
  ...
  if(scopesettings.source2_measures == 0) display_text(503, 273, "CH2");
  if(scopesettings.source2_measures == 1) display_text(499, 242, "Ref 5");
  if(scopesettings.source2_measures == 2) display_text(562, 242, "Ref 6");
  if(scopesettings.source2_measures == 3) display_text(624, 242, "Ref 7");
  if(scopesettings.source2_measures == 4) display_text(686, 242, "Ref 8");
...
void scope_draw_grid(void)
{
  ...
  if (disp_first_sample>scopesettings.samplecount)   disp_first_sample=scopesettings.samplecount;
  
  //kurzor_position = ((disp_first_sample*100)/554)+4;
  kurzor_position = ((disp_first_sample*100)/3000)+4;
...
// */
  ...
  if(scopesettings.gridbrightness > 3)
  {
    ...
    //Draw the center lines orange if single mode
    if(scopesettings.triggermode == 1) display_set_fg_color(ORANGE_COLOR);  else display_set_fg_color(color);  
...
/*
 void u32_to_hexstr(uint32 value, char *buffer) {
...
void make_freq_to_text(char *dst, uint32 f)
{
  char buf[16];
  ...
  if (f < 1000) 
  {
    ...
    else if (f < 1000000) 
    {
      ...
      else 
      {
        ...
        //dst[len++] = '0' + ((r / 1000) % 10);   // tretia
        //dst[len++] = '0' + ((r / 100) % 10);    // štvrtá
        //dst[len++] = '0' + ((r / 10) % 10);     // piata
  ...
  align_text(dst, 8, 2);
  //centering_text(dst, 5);
}

//==============================================================================

void make_freq_to_long_text(char *dst, uint32 f)
{
  char buf[16];
  dst[0] = 0;         // clear buffer
...
void make_value_to_text(char *dst, uint32 d, char c)
{
  char buf[16];
  char tmp[2];
  tmp[0] = c;
...
void make_volt_to_long_text(char *dst, uint32 f)
{
  char buf[16];
  dst[0] = 0;         // clear buffer
...
void centering_text(char *dst, uint8 width)
{
  ...
  // move BACK from the end so we don't overwrite anything
    for (int i = len; i >= 0; i--) dst[i + shift] = dst[i];
      
  // add spaces to the beginning
  for (int i = 0; i < shift; i++) dst[i] = ' ';     
}
*/
void align_text(char *dst, uint8 width, uint8 mode)
{
  //ALIGN_LEFT = 0,  ALIGN_CENTER = 1, ALIGN_RIGHT = 2
    ...
    int left_spaces = 0;
    int right_spaces = 0;
    int free_space = width - len;

    switch (mode)
    {
        case 0:
            left_spaces = 0;
            right_spaces = free_space;
            break;

        case 1:
            left_spaces = free_space / 2;
            right_spaces = free_space - left_spaces;
            break;    
            
        case 2:
            left_spaces = free_space;
            right_spaces = 0;
            break;
    }

    // posun textu doprava o left_spaces
    for (int i = len; i >= 0; i--) dst[i + left_spaces] = dst[i];
        
    // doplnenie medzier na začiatok
    for (int i = 0; i < left_spaces; i++) dst[i] = ' ';
        
    // doplnenie medzier na koniec
    for (int i = 0; i < right_spaces; i++) dst[left_spaces + len + i] = ' ';

    dst[width] = 0;  // ukončenie
}

//==============================================================================

void format_version_full(uint32 value, char *buffer) 
{
    int idx = 0;
    ...
    for (int i = 3; i >= 0; i--) {
        ...
        if (byte >= 100) {
            ...
            buffer[idx++] = '0' + byte / 10;
            buffer[idx++] = '0' + byte % 10;
        } else { buffer[idx++] = '0' + byte; }
    }
    buffer[idx] = '\0';
}

void extract_version_string(uint32 word1, uint32 word2, char *buffer) 
{ 
